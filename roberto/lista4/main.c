...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (v[i] == y) {
      printf("%d tambem pertence ao vetor!\n", y);
      yIndex = i;
    }
    printf("\n");
    // trocando x e y de lugar no vetor
  ...
  printf("Trocando %d e %d de lugares: \n", x, y);
  for (int i = 0; i < size; i++)
    printf("%d\n", v[i]);
...
int comp(const void *a,
         const void *b) { // funcao que define a maneira como qsort irá ordernar
